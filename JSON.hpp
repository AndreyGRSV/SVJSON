...
#include <vector>
    ...
    (!defined _MSC_VER || (defined(_MSC_VER) && _MSC_VER < 1900))
#include <map>
namespace sag {
...
template <typename T> struct JSONEmpty { static T m_Empty; };
...
private:
  template <class Ti>
  typename enable_if<is_same<Ti, std::string>::value, std::string>::type
  getTypeValue() const;
  template <class Ti>
  typename enable_if<is_same<Ti, const char *>::value, const char *>::type
  getTypeValue() const;
  template <class Ti>
  typename enable_if<is_same<Ti, bool>::value, bool>::type getTypeValue() const;
  template <class Ti>
  typename enable_if<is_same<Ti, double>::value, double>::type
  getTypeValue() const;
...
public:
  JSONElement(const std::string &sJSON, std::size_t pos, Type type)
      : m_Type(type), m_Pos(pos), m_sJSON(sJSON) {}
  JSONElement(Type type)
      : m_Type(type), m_Pos(0), m_sJSON(m_EmptyString.m_Empty) {}
  virtual ~JSONElement() {}
...
public:
  JSONStaticString(const std::string &sJSON, std::size_t pos,
                   const char *static_value)
      : JSONElement(sJSON, pos, tStaticString), m_pValue(static_value) {}
...
public:
  JSONString(const std::string &sJSON, std::size_t pos)
      : JSONElement(sJSON, pos, tString) {}
...
template <typename T> struct NumberData { static const char *m_pValue; };
...
public:
  JSONNumber(const std::string &sJSON, std::size_t pos)
      : JSONElement(sJSON, pos, tNumber), m_Value(0) {}
  ...
  virtual std::size_t parse() {
    ...
    // else
    //	m_Value = std::atol (m_sJSON.substr(start_pos, m_Pos -
    //start_pos).c_str());
...
protected:
  ...
  JSONElement *ParseType(std::size_t start_pos, const char *pStaticValue);
...
public:
  ...
  JSONGroup(const std::string &sJSON, std::size_t pos, Type type)
      : JSONElement(sJSON, pos, type) {}
};
...
public:
  JSONArray(JSONArray &arr)
      : JSONGroup<void>(arr.m_sJSON, arr.m_Pos, tArray),
        m_Values(arr.m_Values) {
    arr.m_Values.clear();
  }
#if __cplusplus >= 201103L || (defined(_MSC_VER) && _MSC_VER > 1900)
  JSONArray(JSONArray &&arr) noexcept
      : JSONGroup<void>(arr.m_sJSON, arr.m_Pos, tArray) {
    m_Values = std::move(arr.m_Values);
  }
#endif
  JSONArray() : JSONGroup<void>() {}
  JSONArray(const std::string &sJSON, std::size_t pos)
      : JSONGroup<void>(sJSON, pos, tArray) {
    m_Values.reserve(20);
  }
  virtual ~JSONArray() {
  ...
  const JSONElement &operator[](const ArrayType::size_type _idx) const {
    ...
    const JSONElement *pElement =
        const_cast<std::vector<const JSONElement *> &>(m_Values)[_idx];
    if (pElement)
      return *pElement;
    return get_EmptyArray();
  }
...
public:
  JSONObject(JSONObject &cObj)
      : JSONGroup<void>(cObj.m_sJSON, cObj.m_Pos, tObject),
        m_Values(cObj.m_Values) {
    cObj.m_Values.clear();
  }
#if __cplusplus >= 201103L || (defined(_MSC_VER) && _MSC_VER > 1900)
  JSONObject(JSONObject &&cObj) noexcept
      : JSONGroup<void>(cObj.m_sJSON, cObj.m_Pos, tObject) {
    m_Values = std::move(cObj.m_Values);
  }
#endif
  JSONObject() : JSONGroup<void>() {}
  JSONObject(const std::string &sJSON, std::size_t pos)
      : JSONGroup<void>(sJSON, pos, tObject) {}
  virtual ~JSONObject() {
    cDeleteValue dv;
...
template <class Ti>
typename enable_if<is_same<Ti, std::string>::value, std::string>::type
JSONElement::getTypeValue() const {
...
template <class Ti>
typename enable_if<is_same<Ti, bool>::value, bool>::type
JSONElement::getTypeValue() const {
  if (this->m_Type != tStaticString)
    return false;
...
template <typename T>
template <typename valueT>
JSONElement *JSONGroup<T>::ParseType(std::size_t start_pos,
...
template <typename T>
template <typename valueT>
JSONElement *JSONGroup<T>::ParseType(valueT &pValue) {
  m_Pos = pValue.parse();
  if (m_Pos == std::string::npos)
    return NULL;
#if __cplusplus >= 201103L || (defined(_MSC_VER) && _MSC_VER > 1900)
